...
def LSTM(df):
    ...
    model = Sequential([
        ...
        LSTM(50),
        Dropout(0.2),
        Dense(1)  # Predicting the next day's price
    ])
    ...
    prices_scaled = df[''].fit_transform(df['price'].values.reshape(-1, 1))
    sentiment_scaled = self.sentiment_scaler.fit_transform(df['sentiment'].values.reshape(-1, 1))
        
        # Combine features
    data = np.hstack((prices_scaled, sentiment_scaled))
